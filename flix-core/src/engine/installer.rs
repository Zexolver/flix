...
pub fn install(
    url: &str, 
    shared: SharedArgs, 
    _use_release: bool, // We will use this when we build the GitHub Release scraper
    _is_default: bool, 
    ...
    let commit_hash = match git_manager::fetch_and_checkout(url, &temp_dir, git_ref) {
        Ok(hash) => hash,
    ...
    if let Some(bin_file) = builder::detect_and_build(&temp_dir, &package_name, shared.quiet) {
        ...
        let mut final_tags = shared.tags.clone();
